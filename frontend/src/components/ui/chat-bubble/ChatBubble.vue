...
import { AlertCircle, Check, CheckCheck, Loader2 } from '@lucide/vue'
import { cn } from '@/components/brutx/shared/utils'
import { chatAvatarVariants, chatBubbleVariants } from './chat-bubble-variants'
...
const contentWrapperClass = computed(() =>
    cn('flex flex-col gap-1 min-w-0', isSent.value ? 'items-end' : 'items-start')
)
...
const statusClass = computed(() => {
    ...
    switch (props.message.status) {
        case 'sending':
            return 'text-brutal-fg/50 animate-spin'
        case 'sent':
            return 'text-brutal-fg/50'
        case 'delivered':
            return 'text-brutal-fg/70'
        case 'read':
            return 'text-brutal-primary'
        case 'failed':
            return 'text-brutal-destructive'
        default:
            return ''
    }
})
...
<template>
    <div :class="wrapperClass">
        <!-- Avatar -->
        <div v-if="showAvatar && !isSystem" :class="avatarClass" :title="message.name">
            <img
