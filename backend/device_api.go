...
func (s *Server) DeviceRegisterHandler(w http.ResponseWriter, r *http.Request) {
	...
	if existing, ok := s.devices.Get(req.ID); ok {
		if existing.DisplayName != req.DisplayName && s.devices.NameExists(req.DisplayName) {
			WriteError(w, http.StatusConflict, "DEVICE_NAME_EXISTS", map[string]interface{}{"name": req.DisplayName})
			return
		}
	} else if s.devices.NameExists(req.DisplayName) {
		WriteError(w, http.StatusConflict, "DEVICE_NAME_EXISTS", map[string]interface{}{"name": req.DisplayName})
		return
	}
...
func (s *Server) DeviceRenameHandler(w http.ResponseWriter, r *http.Request) {
	...
	req.DisplayName = strings.TrimSpace(req.DisplayName)
	if req.DisplayName == "" {
		WriteError(w, http.StatusBadRequest, "DEVICE_NAME_REQUIRED", nil)
		return
	}
	if device.DisplayName != req.DisplayName && s.devices.NameExists(req.DisplayName) {
		WriteError(w, http.StatusConflict, "DEVICE_NAME_EXISTS", map[string]interface{}{"name": req.DisplayName})
		return
	}
	...
	if s.db != nil {
		_ = UpdateDeviceName(s.db, req.ID, req.DisplayName)
	}
...
func (s *Server) DeviceRemoveHandler(w http.ResponseWriter, r *http.Request) {
	...
	if !s.devices.Remove(req.ID) {
		WriteError(w, http.StatusNotFound, "DEVICE_NOT_FOUND", nil)
		return
	}
	if s.db != nil {
		_ = DeleteDevice(s.db, req.ID)
	}
	if s.events != nil {
		s.events.Publish(DeviceEvent{Type: "DEVICE_REMOVED", Data: Device{ID: req.ID}})
	}
